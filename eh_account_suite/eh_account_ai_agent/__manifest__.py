...
{
    'name': 'AI Agent Layer',
    'summary': 'Deterministic, no-API-key AI helpers for Odoo 19 Community accounting: journal entry anomaly detection, budget-vs-actual variance commentary, and a collections next-action dunning ladder. Odoo journal entry anomaly detection, weekend posting control, just-under-threshold structuring detection, reversal-pair wash candidate flagging, budget vs actual variance commentary, collections dunning next action, pluggable LLM provider registry, bring-your-own-key cloud and self-hosted local LLM support, rule-based fraud heuristics, deterministic AI with provider fallback.',
