...
class EhApprovalRequest(models.Model):
    ...
    def _eh_user_eligible_for_step(self, user, step_index):
        ...
        if user in step.approver_ids:
            return True
        if step.group_id and step.group_id in user.group_ids:
            return True
    ...
    def _eh_schedule_step_activity(self):
        ...
        if not todo:
            return
        for user in self.pending_group_id.user_ids[:5]:  # cap to avoid spam
    ...
    def _eh_post_reminder(self, manual=False):
        ...
        if not self.pending_group_id:
            return
        users = self.pending_group_id.user_ids.filtered('active')
    ...
    def _eh_escalate_to_group(self, target_group, manual=False):
        ...
        """
        self.ensure_one()
        users = target_group.user_ids.filtered('active')
        partner_ids = users.mapped('partner_id').ids
        body = _(
