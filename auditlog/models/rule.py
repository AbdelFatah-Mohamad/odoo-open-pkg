...
class AuditlogRule(models.Model):
    ...
    model_ids = fields.Many2many(
        "ir.model",
        "auditlog_rule_ir_model_rel",
        "rule_id",
        "model_id",
        string="Models",
        help="Select models for which you want to generate log.",
        index=True,
    )
    ...
    def _patch_methods(self):
        ...
        for rule in self:
            if rule.state != "subscribed":
                continue
            for mod in rule.model_ids:
                if not self.pool.get(mod.model):
                    continue
                model_cache[mod.model] = mod.id
                model_model = self.env[mod.model]
                # CRUD
                # -> create
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "model_ids" not in vals or not vals["model_ids"]:
                raise UserError(_("No models defined to create line."))
        new_records = super(AuditlogRule, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        if "model_ids" in vals:
            if not vals["model_ids"]:
                raise UserError(_("Field 'model_ids' cannot be empty."))
    ...
    def subscribe(self):
        ...
        for rule in self:
            for mod in rule.model_ids:
                domain = "[('model_id', '=', %s), ('res_id', '=', active_id)]" % (mod.id)
                vals = {
                    "name": _("View logs"),
                    "res_model": "auditlog.log",
                    "binding_model_id": mod.id,
                    "domain": domain,
                }
